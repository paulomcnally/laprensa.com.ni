...
echo do_shortcode('<hr>[doap_heading style="modern-1-blue" size="20" align="left" margin="0" class="fp-title-bar"]<a href="http://www.laprensa.com.ni/lptv"><div class="title-left">VIDEOS DESTACADOS</div><div class="twodots"></div></a>[/doap_heading]');

//echo do_shortcode('<div style="position:relative;left:0px;">[doap_posts template="templates/box-loop-videos2.php" posts_per_page="6" tax_term="19" tax_operator="0" order="desc" post_parent=" " ignore_sticky_posts="no"]</div>'); 
$args = array( 'category__in' => 19, 'posts_per_page' => 6, 'meta_query' => array(array('key' => 'destacado','value' => true,'type' => 'BOOLEAN')), 'date_query' => array(array('column' => 'post_date', 'after' => '1 month ago')) );
$the_query = new WP_Query( $args );
include(STYLESHEETPATH . '/templates/lptv-box-loop-videos2.php');
...
$args = array_merge( $wp_query->query_vars, array( 'cat' => 19, 'post__not_in' => $feat_post ));
add_filter('posts_clauses', 'filterEdiciones');
$the_query = new WP_Query( $args );

  if( $the_query->have_posts() ) : 

		$i = 1; while ($the_query->have_posts() && $i < 7) : $the_query->the_post(); 
...
<?php
		...
		$i++; endwhile;
                ...
                echo '<div style="position:relative;float:left;width:100%;background-color:#dbdbdb;padding: 7px 0px 20px 0px;"><div id="lptv-page-nav" style="position:relative;float:right;top:0px;">'; numeric_posts_nav(); echo '</div></div>';
...
/*
<div style="border:1px;position:relative;">
include(STYLESHEETPATH . '/lptv-page-bottom-ad.php'); 
 </div>
//this goes after the closing div below
