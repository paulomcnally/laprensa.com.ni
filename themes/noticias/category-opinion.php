...
echo do_shortcode('[doap_heading style="modern-1-blue" size="20" align="left" margin="0" class="fp-title-bar"]<a href="http://www.laprensa.com.ni/opinion/" title="Use los botones de abajo para navegar por las categorías para este día en la historia."><div class="title-left">OPINION: '.$archive_date.'</div><div class="line-container"><div class="line"></div></div></a>[/doap_heading]');
} else {
echo do_shortcode('[doap_heading style="modern-1-blue" size="20" align="left" margin="0" class="fp-title-bar"]<a href="http://www.laprensa.com.ni/opinion/"><div class="title-left">OPINION</div><div class="line-container"><div class="line"></div></div></a>[/doap_heading]'); 
}

//include(STYLESHEETPATH . '/edicion-buttons.php');
include(STYLESHEETPATH . '/edicion-code.php');

//echo do_shortcode('[doap_spacer size=2][doap_animate type="fadeIn"][doap_heading style="modern-1-blue" size="20" align="left" margin="0" class="fp-title-bar"]<a href="http://www.laprensa.com.ni/opinion"><div style="font-family:Stag;" class="title-left">OPINION</div><div class="line-container"><div class="line"></div></div></a>[/doap_heading][/doap_animate]');
...
if ($page_date == $today)
{
$args = array( 'cat' => 23, 'posts_per_page' => 1, 'ignore_sticky_posts' => 1, 'date_query' => array(array('column' => 'post_date', 'after' => $last_month)) );
}
else
{
$args = array( 'cat' => 23, 'posts_per_page' => 1, 'ignore_sticky_posts' => 1, 'year' => $_year, 'monthnum' => $monthnum, 'day' => $day_of_month); 
}
...
if ($page_date == $today)
{
$args = array( 'cat' => 3515, 'posts_per_page' => 1, 'ignore_sticky_posts' => 1, 'date_query' => array(array('column' => 'post_date', 'after' => $last_month)) );
}
else
{
$args = array( 'cat' => 3515, 'posts_per_page' => 1, 'ignore_sticky_posts' => 1, 'year' => $_year, 'monthnum' => $monthnum, 'day' => $day_of_month); 
}
add_filter('posts_clauses', 'filterEdiciones');
$the_query = new WP_Query( $args );
include(STYLESHEETPATH . '/templates/cat-opinion-loop.php');
...
$max_posts = 7;
global $wp_query;
$args = array_merge( $wp_query->query_vars, array( 'cat' => $themaincat, 'date_query' => array(array('column' => 'post_date', 'after' => $last_month)) ) );
add_filter('posts_clauses', 'filterEdiciones');
$the_query = new WP_Query( $args );
$even = 0; 
...
<?php 
			...
			{ 
				...
				$feat_image = 'http://www.laprensa.com.ni/wp-content/uploads/sites/2/2014/07/empt-box.png';
			} 
			...
			if( responsive_pro_get_option( 'archive_post_excerpts' ) ) 
			{
				...
				$the_excerpt = get_doap_excerpt(300,1,1);
