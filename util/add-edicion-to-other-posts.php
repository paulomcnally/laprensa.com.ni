...
$query = 'SELECT post_id FROM ' . $wpdb->postmeta . ' WHERE post_id IN (SELECT ID FROM ' . $wpdb->posts . ' WHERE post_type = "post" and post_status = "publish") and post_id NOT IN (select post_id from ' . $wpdb->postmeta . ' WHERE meta_key IN ("_edicion")) ORDER BY ABS(post_id) DESC LIMIT ' . $limit . ';';
