						...
						foreach ($catdir as $catfileinfo)
						{
					    		if (!$catfileinfo->isDot() && $catfileinfo != "index.shtml" && !substr_count($catfileinfo,"_print"))
							{
								...
								$extension = strtolower(substr(strrchr($catfileinfo, '.'), 1));
									...
									$cat_cari_image = $cat_dir . 'humor_' . $year . $nummonth . $numday . '.gif';
									$cat_post_pic = 'caricatura-' . $year . '-' . $nummonth . '-' . $numday . '.gif';
									//echo $cat_post_body . PHP_EOL;
									if (!file_exists($cat_cari_image))
									{
										$cat_cari_image = $cat_dir . 'humor_.gif';
									}
									if (!file_exists($cat_cari_image))
									{
										$cat_cari_image = $cat_dir . 'humor.gif';
									}
									if (file_exists($cat_cari_image))
									{
										$clean_title = 'Caricatura Del Dia – ' . $year . '-' . $nummonth . '-' . $numday; 
