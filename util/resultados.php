...
foreach ($ligas as $liga )
{
	...
	$equipos_in_liga = $wpdb->get_col( "SELECT DISTINCT meta_value FROM $wpdb->postmeta where meta_key = 'equipo' AND post_id IN (SELECT DISTINCT post_id FROM $wpdb->postmeta where meta_key = 'liga');" );
	foreach ($equipos_in_liga as $el_equipo)
	{
		echo PHP_EOL . PHP_EOL . PHP_EOL . 'EQUIPO ' . $el_equipo . '____________________  liga ' . $liga_name . '_______________________________________';
			$last3_posts = $wpdb->get_col( "SELECT DISTINCT post_id FROM $wpdb->postmeta where meta_key IN ('en_casa', 'visitante') ORDER BY ABS(post_id) DESC LIMIT 3;" );
...
//echo 'HELLO';
echo PHP_EOL . '<div style="top:17px;position:relative;"><img src="http://noticias.laprensa.com.ni/wp-content/themes/noticias/core/icons/' . $deporte_icon . '.png" alt="futbol" width="22" height="22" class="alignleft size-thumbnail wp-image-1146384" /></div>';
...
echo PHP_EOL . '<div class="shawn-marcador-inner" style="width:100%;height:80px;position:relative;float:left;">';
echo PHP_EOL . '<div style="text-align:center;line-height:5;width:130px;height:80px;color:#606060;font:Arial;font-size:22;font-weight:900;position:relative;float:left;">'. $home_meta . '</div>';
...
echo PHP_EOL . '<div style="line-height:1.6;'. $home_color . 'color:#fff;font-family:Arial;font-size:3.5em;font-weight:bold;width:100px;height:80px;text-align:center;vertical-align:middle;position:relative;float:left;">'. $home_score_meta . '</div>';
echo PHP_EOL . '<div style="line-height:1.6;' . $away_color . 'color:#fff;font-family:Arial;font-size:3.5em;font-weight:bold;width:100px;height:80px;text-align:center;vertical-align:middle;position:relative;float:left;">'. $away_score_meta . '</div>';
...
echo PHP_EOL . '<div style="text-align:center;line-height:5;width:130px;height:80px;color:#606060;font:Arial;font-size:22;font-weight:900;position:relative;float:left">'. $away_meta . '</div>';
echo PHP_EOL . '</div>';
echo PHP_EOL . '<div style="clear:both;"></div>';
echo PHP_EOL . '<div style="display:table-cell;border-top: 1px solid #ccc;border-collapse:collapse;width:100%;min-height:10px;color:#606060;font:Arial;font-size:22;font-weight:400;text-align:center;position:relative;float:left">'. $game_title . '</div>';
echo PHP_EOL . '</div>';
