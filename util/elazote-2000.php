...
$start_date = strtotime("2002-10-05");
$end_date = strtotime("2006-03-04");
...
$base_dir = "/var/www/html/laprensa/lp-archivo/public_html/archivo/$year/$thismonth/$numday/elazote/";
echo $year . PHP_EOL;
echo $numday . PHP_EOL;
echo $day . PHP_EOL;
...
echo 'cat_dir EXISTS' . PHP_EOL;
						...
						foreach ($catdir as $catfileinfo)
						{
							echo 'catfileinfo = ' . $catfileinfo . PHP_EOL . PHP_EOL;
					    		if (!$catfileinfo->isDot() && $catfileinfo != "index.html" && !substr_count($catfileinfo,"_print"))
							{
						       		// var_dump($fileinfo->getFilename());
								...
								if ($extension == 'html')
								{
									...
									$cat_pic_stub = str_replace(".html", "", $catfileinfo);
									$cat_cari_image = $cat_dir . str_replace(".html", ".jpg", $catfileinfo);
...
$cat_post_text = mb_convert_encoding($cat_post_text, "UTF-8", "CP1252");
										$title = extractTag($cat_post_text, '', '<FONT color="#990000" face=verdana,arial size=6>','</FONT>');
										...
										$permalink = remove_accents($clean_title );
								                ...
								                $post_status = 'publish';
										//$post_text = extractTag($cat_post_text, '', '<!--texto-->', '</FONT></TD>');
										$post_text = getTextBetween($cat_post_text, '<!--texto-->', '</FONT></TD>');
//echo 'cat_post_text = ' . PHP_EOL . $cat_post_text . PHP_EOL;
//echo 'post_text = ' . PHP_EOL . $post_text . PHP_EOL;
										if (!empty($post_text))
										{
											$post_text = '<p class="archivo_v1_body">' . $post_text . '</p>';
//echo 'not empty post_text = ' . PHP_EOL . $post_text . PHP_EOL;
										}
										$post_text = $post_text . PHP_EOL . $caricatura_html;
//echo 'post_text with manual info = ' . PHP_EOL . $post_text . PHP_EOL;
								                $post = array(
										...
										$query = "SELECT post_id FROM wp_2_postmeta where meta_key = 'lparchivo' and meta_value = '" . $cat_post_file . "' LIMIT 1;"; 
