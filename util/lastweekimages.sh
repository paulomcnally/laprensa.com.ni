...
find . -mmin -7200 -type f > /tmp/noticias-lastweek-files.txt
#cd /home/shawn
cd /tmp
sed -i 's#\./##g' /tmp/noticias-lastweek-files.txt
sed -i '/wpcf7_captcha/d' /tmp/noticias-lastweek-files.txt
#FILEPATH=`dirname $(readlink -f $0)`
FILEPATH=/tmp
echo $FILEPATH
FILE=$FILEPATH/noticias-lastweek-files.txt
LOG=$FILEPATH/noticias-lastweek-files.log
i=0
while read line; do
  if [[ $line != "" ]]; then
    arr[$i]=$line
