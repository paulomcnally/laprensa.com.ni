...
$idnoticias_imported= $wpdb->get_col( "SELECT post_id FROM $wpdb->postmeta where meta_key = 'idnoticia' ORDER BY ABS(post_id) DESC LIMIT $limit;" );
...
$noticias_imported= $wpdb->get_col( "SELECT ID FROM $wpdb->posts WHERE post_status = 'publish' and post_content like '%</div>\n</div>%' ORDER BY ABS(ID) DESC LIMIT $limit;" );
