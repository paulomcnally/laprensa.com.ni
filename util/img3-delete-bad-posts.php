...
try
{
	$dbp2 = new PDO("pgsql:dbname=$pgsql_db;host=$pgsql_hostname", "$pgsql_username", "$pgsql_password" );
       	$sql2 = "SELECT idimagen FROM imagen WHERE imagen LIKE '%600x400%';"; 
...
$query = "SELECT post_id, meta_value as imgid FROM $wpdb->postmeta WHERE meta_key = 'idimagen' and meta_value NOT IN ($imgids) and post_id IN (SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '_wp_attached_file' and meta_value like '%600x400%') ORDER BY abs(post_id) DESC LIMIT $limit;";
