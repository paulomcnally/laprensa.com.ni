...
try
{
	...
	if ($metavals4)
		{ 
		$sql = "select distinct acl.idnoticia from acl WHERE acl.idacl > $last_imported and acl.idacl NOT IN ($metavals4) and acl.idnoticia IN (SELECT idnoticia from noticia) ORDER BY acl.idnoticia ASC limit $limit";	
		}
	else
		{
...
try     {
	...
	$strAcl = '<div class="aside-div">';
		...
		if(!empty($check_acl)) {
			echo "Sidebar import is already running!!!";
