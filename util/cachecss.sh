...
do
  ...
  /usr/bin/s3cmd put --config=/var/www/html/.s3cfg --no-check-md5 --no-encrypt -m text/css -p -P -H --add-header="Expires:`date -u +"%a, %d %b %Y %H:%M:%S GMT" --date "next Year"`" --add-header='Cache-Control:max-age=31536000, public' "/var/www/html/lpmu/wp-content/cache/minify/000002/12b6a/$thumb" "s3://laprensa.com.ni/wp-content/cache/minify/000002/12b6a/$thumb" >> $LOG
