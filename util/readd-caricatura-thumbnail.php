...
$caricaturas_imported= $wpdb->get_col( "SELECT post_id FROM $wpdb->postmeta where meta_key = 'idcaricatura' and post_id NOT IN (SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '_thumbnail_id') ORDER BY ABS(post_id) ASC;" );
...
foreach ($caricaturas_imported as $row)
{
	$post_id = $row;
