...
//$cat_post_text = html_entity_decode($cat_post_text ,ENT_QUOTES,"UTF-8");
...
if($has_div_img)
{
	if ($has_div_info)
	{
		...
		foreach ($div_info_array as $div_info)
			...
			$image['caption'][] = wp_strip_all_tags($div_info);
			$cat_post_text = str_replace($div_info,'',$cat_post_text);
			}
	}
	$foto_array = extractTags($cat_post_text, ' image-','<div class="na-media na','</div>');
	foreach ($foto_array as $foto)
		...
		$cat_post_text = str_replace($foto,"[imported_image_$imgid]",$cat_post_text);
		}
...
$cat_post_text  = str_ireplace("<p>&nbsp;</p>","",$cat_post_text );

echo PHP_EOL . 'cat post text = ' . $cat_post_text . PHP_EOL;
echo PHP_EOL . 'my count= ' . $my_count . PHP_EOL;
