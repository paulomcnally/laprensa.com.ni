...
$noticias_imported= $wpdb->get_col( "SELECT ID FROM $wpdb->posts where post_type = 'attachment' and post_parent <> '' and ID > 499637 and ID < 865706 and ID NOT IN (SELECT post_id from $wpdb->postmeta where meta_key = 'lp_reprocessed') ORDER BY ABS(ID) DESC LIMIT $limit;" );
...
//	$post_term_results = wp_set_post_terms( $post_id, $post_cat, 'category' );
	...
	$i++;
	$xyz = wp_get_attachment_metadata($attach_id);
	$upload_dir = wp_upload_dir();
	$file = $upload_dir['basedir'] . '/' . $xyz['file'];
	echo PHP_EOL . ' file = ' . $file . PHP_EOL;
	$attach_data = wp_generate_attachment_metadata( $attach_id, $file );
        wp_update_attachment_metadata( $attach_id, $attach_data );
//	s3CopyImage($attach_id);
	$reprocessed_image = add_post_meta( $attach_id, 'lp_reprocessed', '1', true );
}




echo PHP_EOL . $i . ' Attachments Updated' . PHP_EOL;
