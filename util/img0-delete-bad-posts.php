...
$deletefiles = implode('", "', file('emptyfiles.txt'));
$deletefiles = preg_replace( "/\r|\n/", "", $deletefiles );
$processedfiles = implode('", "', file('deletedfiles.txt'));
$query = 'SELECT post_id, meta_value as filename FROM ' . $wpdb->postmeta . ' WHERE meta_value IN ("' . $deletefiles . '") and meta_value NOT IN ("' . $processedfiles . '") and post_id IN (SELECT post_id from ' . $wpdb->postmeta . ' WHERE meta_key = "idimagen") ORDER BY abs(post_id) DESC LIMIT ' . $limit;;
...
foreach ($noticias_imported as $row)
{
	...
	$deletefile = fopen('deletedfiles.txt', 'a');
