...
$noticias_imported= $wpdb->get_col( "SELECT ID FROM $wpdb->posts where post_type = 'attachment' and post_parent <> '' and ID > 499637 and ID < 865706 and ID NOT IN (SELECT post_id from $wpdb->postmeta where meta_key = 'lp_reprocessed') ORDER BY ABS(ID) ASC LIMIT $limit;" );
