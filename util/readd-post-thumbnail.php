...
$noticias_imported= $wpdb->get_col( "SELECT post_id FROM $wpdb->postmeta where meta_key IN ('idnoticia','idarticulo') and post_id IN (SELECT post_parent FROM $wpdb->posts where post_type = 'attachment' and post_parent <> 0) and post_id NOT IN (SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '_thumbnail_id') ORDER BY ABS(post_id) ASC LIMIT $limit;" );
