...
$numday = date("d", $b);
$year = date("Y", $b);
$editions_to_import = (isset($editions)) ? $editions : '';
$categories_to_import = (isset($sections)) ? $sections : '';
//$categories_to_import = "2,3,5,7,10,11,13";
...
try	{
	...
	if ($metavals4)
		...
		$sql = "SELECT articulo.idarticulo, seccionsuplemento.seccionsuplemento as antetitulo, seccionsuplemento.idsuplemento as idseccion, articulo.articulo as noticia, articulo.resumen, articulo.texto, articulo.claves, articulo.intro, articulo.antetitulo, articulo.ubicacion, articulo.idedicionsuplemento, edicionsuplemento.edicionsuplemento as creacion  FROM articulo, seccionsuplemento, edicionsuplemento WHERE articulo.idarticulo NOT IN ($metavals4) and seccionsuplemento.idseccionsuplemento = articulo.idseccionsuplemento and articulo.idedicionsuplemento =  edicionsuplemento.idedicionsuplemento and seccionsuplemento.idsuplemento IN ($categories_to_import) and articulo.idedicionsuplemento IN ($editions_to_import) ORDER BY articulo.idarticulo desc limit $limit;";
		}
	else
		{
		$sql = "SELECT articulo.idarticulo, seccionsuplemento.seccionsuplemento as antetitulo, seccionsuplemento.idsuplemento as idseccion, articulo.articulo as noticia, articulo.resumen, articulo.texto, articulo.claves, articulo.intro, articulo.antetitulo, articulo.ubicacion, articulo.idedicionsuplemento, edicionsuplemento.edicionsuplemento as creacion  FROM articulo, seccionsuplemento, edicionsuplemento WHERE seccionsuplemento.idseccionsuplemento = articulo.idseccionsuplemento and articulo.idedicionsuplemento =  edicionsuplemento.idedicionsuplemento and seccionsuplemento.idsuplemento IN ($categories_to_import)  and articulo.idedicionsuplemento IN ($editions_to_import) ORDER BY articulo.idarticulo desc limit $limit;";
		}
...
//usleep(50000);
			...
			$new_posts[] = '<div style="clear:both;"><a href="http://noticias.laprensa.com.ni/index.php?p=' . $post_id . '">Post # ' . $post_id . '</a></div>' . PHP_EOL;
//usleep(50000);
		}
	}
$i++;
...
echo '<br>' . PHP_EOL . $xx . ' ARTICLES CREATED' . PHP_EOL;
