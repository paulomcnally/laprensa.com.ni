...
foreach ($noticias_imported as $row)
{
	...
	try
       	{
       		...
       		$sql3 = "SELECT noticia.orden, edicion.edicion from noticia, edicion WHERE noticia.idedicion = edicion.idedicion and noticia.idnoticia = $idnoticia limit 1;";
       		foreach ($dbp3->query($sql3) as $row3)
       		{
			$articleedition = $row3['edicion'];
			$articlepeso = $row3['orden'];
			$pm_edicion_result = add_post_meta( $post_id, 'edicion', $articleedition, true );
			$pm_peso_result = add_post_meta( $post_id, 'peso', $articlepeso, true );
       		}
       	$dbp3 = null;
       	}
	...
	{
		echo 'FAILED!  Edicion Postmeta RESULT = ' . $pm_peso_result . '       idnoticia = ' . $idnoticia . '       post_id = ' . $post_id . PHP_EOL;
	}
	$i++;
//	//usleep(50000);
}
