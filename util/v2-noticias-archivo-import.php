...
$start_date = strtotime("2009-09-10");
$end_date = strtotime("2009-09-11");
...
$remove_txt = array('<img src="/img/cuadritonaranja.gif" width="6" height="6" alt=".">','<img src="/img/cuadritonaranja.gif" width="6" height="6">', 'xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:user="http://my_domain_name/my_namespace"');
$q = 0;
$i = $start_date;
...
$cat_dir = $base_dir . $fileinfo . "/";
				...
				if (file_exists($cat_dir)) {
					...
					foreach ($catdir as $catfileinfo) {
					    	if (!$catfileinfo->isDot() && $catfileinfo != "index.shtml") {
       		// var_dump($fileinfo->getFilename());
	...
	$title_start = '<span class="titulo_nota_nacionales">';
	$title_end = '</span>';
...
$cat_post_title = mb_convert_encoding($cat_post_title, "UTF-8", "ISO-8859-1");
$cat_post_title = wp_strip_all_tags($cat_post_title);
$permalink = remove_accents($cat_post_title);
...
$cat_post_text = mb_convert_encoding($cat_post_text, "UTF-8", "CP1252");
	...
	foreach($remove_txt as $remove_me) {
		...
		echo PHP_EOL . 'occurs - ' . substr_count( $cat_post_text , $remove_me) . PHP_EOL;
		$cat_post_text = str_replace($remove_me, "", $cat_post_text);
		echo PHP_EOL . $remove_me . PHP_EOL;
		...
		//$temp_post_text = removeTag($cat_post_text,'trsp.gif','<table','/table>');
		//$cat_post_text = $temp_post_text;
...
$pic_start = 'src="fotos/';
        $pic_end = '"';
        $pic_pos_start = strpos(strtolower($cat_post_text), $pic_start);
$pic_pos_start = $pic_pos_start + strlen($pic_start);
$pic_pos_end_1 = strpos(strtolower($cat_post_text), $pic_end, $pic_pos_start);
//$pic_pos_end_2 = strpos(strtolower($cat_post_text), $pic_end, ($pic_pos_end_1 + 1 ));
$pic_pos_end = $pic_pos_end_1 - $pic_pos_start; 

$cat_pic_file = substr( $cat_post_text, $pic_pos_start, $pic_pos_end );
//        $pic_sub_start = strlen($pic_start);
//        $pic_sub_end = strlen($cat_pic) - strlen($pic_end) - strlen($pic_start);
//        $cat_pic_file = substr($cat_pic, $pic_sub_start, $pic_sub_end);
