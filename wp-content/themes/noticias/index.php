...
global $wp_query;
include(STYLESHEETPATH . '/templates/economia.php');
wp_reset_query();
$max_posts = 5;
$args = array( 'post_type' => 'post', 'posts_per_page' => $max_posts, 'meta_query' => array( array( 'key' => 'portada', 'value' => 1, 'type' => 'NUMERIC' ) ), 'post__not_in' => $feat_post, 'date_query' => array(array('column' => 'post_date', 'after' => $last_month)) );
...
//	include('/srv/www/uploads/common/marcadorhome.php');
?>
<!-- mclude W3TC_DYNAMIC_SECURITY -->
<?php include('/srv/www/uploads/common/marcadorhome.php');?>
<!-- /mclude W3TC_DYNAMIC_SECURITY -->
<?php
	echo '</div>';
}
	...
	echo '<div style="margin:0px 4px 0px 4px;width:300px;position:relative;float:left;z-index:1000;">'; ?>
...
if ($even > 3)
{
	echo '<div id="story-no-clear" style="margin:10px 0px 10px 0px;width:100%;max-width:530px;clear:initial;">';
	echo '<div style="padding:4px;">';
	$max_width = 'width:530px;'; //width:376px;
	include(STYLESHEETPATH . '/templates/normal.php');
	echo '</div>';
	echo '</div>';
	echo '<div style="clear:both;"></div>';
	include(STYLESHEETPATH . '/ad-336x280-portada-middle.php');
}
if ($even == 200)
{
?>
...
$max_posts = 3;
$args = array( 'post_type' => 'post', 'posts_per_page' => $max_posts, 'cat' => 17, 'meta_key' => '_thumbnail_id', 'post__not_in' => $feat_post, 'date_query' => array(array('column' => 'post_date', 'after' => $last_month)) );
add_filter('posts_clauses', 'filterEdiciones');
...
wp_reset_query();
$max_posts = 1;
$args = array( 'post_type' => 'post', 'posts_per_page' => $max_posts, 'cat' => 63721, 'meta_key' => '_thumbnail_id', 'post__not_in' => $feat_post, 'date_query' => array(array('column' => 'post_date', 'after' => $last_month)) );
//add_filter('posts_clauses', 'filterEdiciones');
//add_action('pre_get_posts', 'alter_query_edicion');
$the_query = new WP_Query( $args );
//var_dump($the_query);
include(STYLESHEETPATH . '/templates/teaser-home-center-col.php');
echo do_shortcode('[doap_heading style="modern-1-blue" size="20" align="left" margin="0" class="fp-title-bar"]<a href="/espectaculo"><div id="des-sec" class="title-left">ESPECTÁCULO</div><div id="des-dot" class="twodots"></div></a>[/doap_heading]');
$max_posts = 4;
$args = array( 'post_type' => 'post', 'posts_per_page' => $max_posts, 'cat' => 22, 'meta_key' => '_thumbnail_id', 'post__not_in' => $feat_post, 'date_query' => array(array('column' => 'post_date', 'after' => $last_month)) );
add_filter('posts_clauses', 'filterEdiciones');
wp_reset_query();
$the_query = new WP_Query( $args );
include(STYLESHEETPATH . '/templates/teaser-home-center-col.php');
echo do_shortcode('[doap_heading style="modern-1-blue" size="20" align="left" margin="0" class="fp-title-bar"]<a href="/tecnologia"><div id="des-sec" class="title-left">TECNOLOGÍA</div><div id="des-dot" class="twodots"></div></a>[/doap_heading]');
$max_posts = 3;
$args = array( 'post_type' => 'post', 'posts_per_page' => $max_posts, 'cat' => 991, 'meta_key' => '_thumbnail_id', 'post__not_in' => $feat_post, 'date_query' => array(array('column' => 'post_date', 'after' => $last_month)) );
add_filter('posts_clauses', 'filterEdiciones');
wp_reset_query();
$the_query = new WP_Query( $args );
include(STYLESHEETPATH . '/templates/teaser-home-center-col.php');
...
<?php
$args = array( 'post_type' => 'post', 'posts_per_page' => 4, 'meta_key'=> '_thumbnail_id', 'meta_query' => array(array('key' => 'bloque_destacado_portada','value' => true,'type' => 'BOOLEAN')), 'post_status' => 'publish', 'date_query' => array(array('column' => 'post_date', 'after' => $last_month)) );
add_filter('posts_clauses', 'filterEdiciones');
wp_reset_query();
$the_query = new WP_Query( $args );
$even = 0;
