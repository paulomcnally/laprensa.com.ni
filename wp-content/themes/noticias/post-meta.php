...
if ( $_GET['archive_page'] == 1 ) { $ap = '?archive_page=1'; } else { $ap = ''; }
...
if ($autor_meta)
{
	$autor = '<p id="autor-single" class="autor">Por: ' . $autor_meta . '</p>';	
}
else
{
$autor ='';
...
<?php //echo do_shortcode('[doap_heading style="modern-1-blue" size="20" align="left" margin="0" class="fp-title-bar"]<a href="http://www.laprensa.com.ni/'.strtolower($single_cat_title).'/"><div class="title-left">'.mb_strtoupper($single_cat_title).'</div><div class="twodots"></div></a>[/doap_heading]'); ?>

<?php echo '<div id="entry-title" class="single-title">'.get_the_title($ID).'</div>'; ?>
	<div class="antetitulo subtitulo"><?php echo get_post_meta( get_the_ID(), 'subtitulo', true ); ?></div>
...
<div class="post-meta">
<?php $autor_1_value = get_post_meta( get_the_ID(), 'autor', true ); if( ! empty( $autor_1_value ) ) { echo '<span id="autor-single" style="position:relative;float:left;margin:10px 0;">Por: '.$autor_1_value.'</span>'; } ?>
