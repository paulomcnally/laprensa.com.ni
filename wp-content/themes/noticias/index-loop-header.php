...
if( is_category() || is_tag() || is_author() || is_date() ) {
	...
	<h6 class="title-archive">
		<?php
		if( is_day() ) :
			printf( __( 'Notas diarios: %s', 'responsive' ), '<span>' . get_the_date() . '</span>' );
		elseif( is_month() ) :
			printf( __( 'Notas mensual: %s', 'responsive' ), '<span>' . get_the_date( 'F Y' ) . '</span>' );
		elseif( is_year() ) :
			printf( __( 'Notas anual: %s', 'responsive' ), '<span>' . get_the_date( 'Y' ) . '</span>' );
		else :
			//_e( 'Deportes', 'responsive' );
		endif;
		?>
	</h6>
<?php
