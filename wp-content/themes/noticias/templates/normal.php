...
if ($show_comments)
{
	if ($show_cat_in_comments)
	{
		...
		$posted_time = date('g:i A',$post_time);
		$posted_hour = date('H:i',$post_time);
		//echo ' ' . $posted_hour . ' ' . $posted_time;
		if (date("Y-m-d",$post_time) == date("Y-m-d"))
		{
       			if ($posted_hour > '01:00')
       			{ 
                		echo PHP_EOL . ' | <span style="color:red">Último minuto</span> ' . $posted_time . PHP_EOL;
			} 
		} 
		else
		{
			echo date("Y-m-d",$post_time) . ' ' . date("Y-m-d");
		}
...
$feat_image_array = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), 'Video-Poster-640x360' );
