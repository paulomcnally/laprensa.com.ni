...
<?php echo do_shortcode('[xyz-ips snippet="tcp-product-grid-layout"]'); ?>
					...
					<?php if ( tcp_has_post_thumbnail() ) :  ?>
						<div class="tcp-product-thumbnail">
							<a class="tcp_size-<?php echo $image_size;?>" href="<?php tcp_the_permalink(); ?>">
								<?php /*the_post_thumbnail('thumbnail');*/ //tcp_the_thumbnail( get_the_ID(), 0, 0, $image_size, array( 'alt' => get_the_title(), 'title' => get_the_title() ) ); ?>
								<?php if ( function_exists( 'the_post_thumbnail' ) ) the_post_thumbnail( $image_size, array( 'alt' => get_the_title(), 'title' => get_the_title() ) ); ?>
								<?php //the_post_thumbnail('thumbnail', array('src'=> $src, 'class' => "responsive-image attachment-$image_size", 'alt' => trim(strip_tags( $wp_postmeta->_wp_attachment_image_alt )))); ?>
							</a>
