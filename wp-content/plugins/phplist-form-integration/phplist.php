...
function phplist_widget_init() {
	...
	function phplist_widget($args) {
		...
		if(phplist_check_input()) // If the input check returns true (ie. there has been a subscriber & input is ok)
		{
			   if(phplist_subscribe($_POST, true) === true) {
			   		echo("<h4>" . __('Gracias por Suscribirse','phplist') . "</h4>");
			   } else {
			   		echo("<h3>" . __('An Error Occurred','phplist') . "</h3>");
			   		echo("<p>" . __("No se pudo añadir su correo electrónico a nuestra lista. Por favor, envíenos un correo electrónico a <a href=\"mailto:support@doap.com\">support@doap.com</a> haciéndonos saber lo que pasó y nos aseguraremos de que te dan en la lista.",'phplist'));
			   }
		}
	...
	function phplist_widget_control() {
		...
		if (!is_array($options)) {
			$options = array(
				'title' => __("Suscríbase a nuestro boletín.", 'phplist')
			);
		}
		...
		print('
			<p style="text-align:right;"><label for="phplist_widget_title">' . __('Titulo:') . ' <input style="width: 200px;" id="phplist_widget_title" name="phplist_widget_title" type="text" value="'.$title.'" /></label></p>
...
// 3) Now simulate post to subscriber form.
 ...
 // No longer required  $post_data["list[$lid]"] = "signup";
   ...
   return(0);
	...
	if(!$widget){
 		echo ('<h3>' . __('Gracias por suscribirse a nuestra lista de correo electrónico.') .'</h3>');
 	} else {
		curl_close($ch);
		return true;
	}
...
function phplist_generate_list_elements () {
	...
	{
		$lists_mes = '<br><span style="caption">' . __('Por favor, seleccione los boletines que desea inscribirse para:','phplist') . '</span><br>';
