...
function tptn_pop_posts( $args ) {
	...
	if ( $results ) {
		...
		foreach ( $results as $result ) {
			...
			//$title_length = 40;
			//$long_title_length = 160;
			$title = tptn_max_formatted_content( get_the_title( $result->ID ), $title_length );
			$longtitle = tptn_max_formatted_content( get_the_title( $result->ID ), $long_title_length );
			...
			if ( ! $p_in_c ) {
				...
				if ( 'after' == $post_thumb_op ) {
					...
					$output .= '<span>' . $title . '</span>'; // Add title if post thumbnail is to be displayed after
				...
				if ( 'inline' == $post_thumb_op || 'text_only' == $post_thumb_op ) {
					$output .= '<span class="tptn_after_thumb" style="height:30px;">';
					$output .= '<a href="' . get_permalink( $result->ID ) . '" rel="bookmark' . $rel_attribute . '" ' . $target_attribute . 'class="tptn_link"' . ' title="' . $longtitle . '">'; // Add beginning of link
					//$output .= '<span class="su-posts su-posts-list-loop">' . $title . '</span>'; // Add title when required by settings
					$output .= '<span>' . $title . '</span>'; // Add title when required by settings
					//$output .= '<img class="thumbnail" src="http://www.laprensa.com.ni/wp-content/uploads/sites/2/2014/10/Screen-Shot-2014-10-13-at-2.39.55-PM.png" style="position:relative;top:-2px;max-width:25px;max-height:18px;" alt=""><span>' . $title . '</span>'; // Add title when required by settings
					$output .= '</a>'; // Close the link
				}
				if ( $show_author ) {
...
function tptn_show_daily_pop_posts() {
	...
	if ( $tptn_settings['d_use_js'] ) {
		echo '<script type="text/javascript" src="' . $tptn_url . '/top-10-daily.js.php?widget=0"></script>';
	} else {
...
function tptn_default_options() {
	...
	$tptn_settings = array (
		...
		'before_list_item' => '<li style="font-weight:100;margin-bottom: 25px;">',		// Before each list item
