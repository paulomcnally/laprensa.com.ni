...
class pageFlip_plugin_base
{
	...
	function init()
	{
		...
		$this->functions = new pageflipFunctions();
		$this->html = new pageflipHTMLPart();

		
		$this->layouts[1] = new pageflipLayout( 1 );
		$this->layouts[1]->addArea( 0, 0, 0, 1, 1 );
		$this->layouts[2] = new pageflipLayout( 2 );
		$this->layouts[2]->addArea( 0, 0, 0, 1, 0.5 );
		$this->layouts[2]->addArea( 1, 0, 0.5, 1, 0.5 );
		$this->layouts[3] = new pageflipLayout( 3 );
		...
		$this->layouts[4] = new pageflipLayout( 4 );
	...
	function main_page()
	{
		...
		if ( !empty($_REQUEST['pageflip_options_update']) )
		{
			if ( !empty($_REQUEST['pageflip_options']['scripts_in_head']) )
	...
	function edit_page()
	{
		...
		$bookID = htmlspecialchars($_REQUEST['id']);
		$pageID = htmlspecialchars($_REQUEST['pageId']);
		$zoom = isset($_REQUEST['zoom']) && $_REQUEST['zoom'] == $tZoomIn ? true : false;
		...
		$navPage = htmlentities($_REQUEST['page'], ENT_COMPAT, 'utf-8');
...
';
	...
	function manage_books()
	{
		...
		if ( !empty($_REQUEST['action']) && $_REQUEST['action'] == __('Page Properties', 'pageFlip') )
		{
			if ( !empty($_REQUEST['save']) )
			{
				$book = new pageflipBook($_REQUEST['id']);
				$book->load();

				$book->pages[$_REQUEST['pageId']]->name = htmlspecialchars($_REQUEST['pageName']);

				if ( trim($_REQUEST['textToWrite']) )
				{
					$book->pages[$_REQUEST['pageId']]->writeText(
						$_REQUEST['textToWrite'],
						array(
							'pageWidth' => $_REQUEST['pageWidth'],
							'pageHeight' => $_REQUEST['pageHeight'],
							'left' => $_REQUEST['textLeft'],
							'top' => $_REQUEST['textTop'],
							'fontFamily' => $_REQUEST['fontFamily'],
							'fontSize' => $_REQUEST['fontSize'],
							'color' => $_REQUEST['color']
						),
						strstr($_REQUEST['zoom'], __('Zoom In', 'pageFlip')) ? true : false
					);
				}
		...
		if( !empty($_REQUEST['thisdo']) ) $_REQUEST['do'] = $_REQUEST['thisdo'];

		$book = NULL;

		if( isset($_REQUEST['actionButton']) )
		{
			switch($_REQUEST['action'])
			{
			 	...
			 	case 'addpage':
			 		$this->add_page($_REQUEST['imageId'], $_REQUEST['type'], $book);
			 		break;
			 	...
			 	case 'Assign Selected Images to Page':
			 		if( count($_REQUEST['images']) > 0 )
			 		{
						...
						foreach ($_REQUEST['images'] as $imageId)
			 				$this->add_page($imageId, $_REQUEST['type'], $book);
			 	...
			 	case 'Assign Images from Gallery':
			 		$this->addPageFromGallery($_REQUEST['galleryId'], $_REQUEST['type']);
			 		unset($_REQUEST['do']);
					break;

			 	case 'uploadimage':
			 		if( ($_REQUEST['do'] == 'New Page') )
			 		{
			 			$imagesId = $this->upload_image( 'New page' );
			 			if( is_array($imagesId) && count($imagesId) > 0 )
			 			{
			 				$book = new pageflipBook($_REQUEST['id']);
			...
			switch( $_REQUEST['do'] )
		 	{
			 	case __('Book Properties', 'pageFlip') :
			 		$this->book_form( $_REQUEST['id'] );
			 		break;

			 	case __('Add Page', 'pageFlip') :
			 	case 'New Page' :
			 		echo $this->functions->printHeader( 'New Page to book #' . $_REQUEST['id'] );
			 		if( isset( $_REQUEST['imageId'] ) && $_REQUEST['action'] == 'Assign Image to Page' && isset( $_REQUEST['actionButton'] ) )
			 			$this->add_page_form( $_REQUEST['id'], $_REQUEST['imageId'], $_REQUEST['type'] );
			 		elseif( ($_REQUEST['action'] == 'uploadimage') && ( count( $imagesId ) == 1 ) )
			 			$this->add_page_form( $_REQUEST['id'], $imagesId[0] );
			 		else
					{
						...
						$text = "							<form method=\"post\" name=\"addGalleryForm\" id=\"addGalleryForm\" action=\"\" style=\"margin:1em 0;\">
						...
						echo $text;
						$this->galleriesList( $_REQUEST['id'] );
						
					}
			 		break;

			 	case 'Upload New Images' :
			 		echo $this->html->uploadImageForm( $_REQUEST['id'] );
			 		break;
				...
				case __('Access', 'pageFlip') :
			 		$this->book_access_form();
				 	break;
	...
	function images()
	{
		...
		switch( $do )
		{
		 	...
		 	case 'Upload Image' : echo $this->html->uploadImageForm( $_REQUEST['bookId'] ); echo '<br />!!!!!!'; break;
		 	case __('Create Book', 'pageFlip') :
		 		global $wpdb;
		 		$gallery = $wpdb->get_row("SELECT * FROM {$this->table_gal_name} WHERE id='{$_REQUEST['galleryId']}' ");
	...
	function book_access_form()
	{	
		echo '<a href="http://pageflipgallery.com/buy/"><img style="margin:60px 0px 0px 50px;" src="'. WP_PLUGIN_URL .'/'. $this->plugin_dir .'/img/access.jpg" /></a>';
	}
	...
	function books_list()
	{
		...
		else foreach( $books as $curBook )
		{
			 ...
			 $book = new pageflipBook( $curBook['id'] );
	...
	function bookPreview( $book = '' )
	{
		if ( empty($book) )
		{
			$book = new pageflipBook( (int)$_REQUEST['bookId'] );
			$ajax = true;
		}
		else
			$ajax = false;
	...
	function replacePages()
	{
		...
		{
			case 'up':
				if ( (int)$_REQUEST['pageId'] > 0 )
				{
					$book->pages[((int)$_REQUEST['pageId'] - 1)]->number++;
					$book->pages[(int)$_REQUEST['pageId']]->number--;
				}
				break;
			case 'down':
				if ( (int)$_REQUEST['pageId'] < $book->countPages )
				{
					$book->pages[(int)$_REQUEST['pageId']]->number++;
					$book->pages[((int)$_REQUEST['pageId'] + 1)]->number--;
				}
	...
	function pagesList()
	{
		...
		$list  = $this->html->headerPreviewPage( (int)$_REQUEST['bookId'] );
	...
	function add_book()
	{
		...
		if ( empty($_REQUEST['bookName']) )
			$_REQUEST['bookName'] = 'unnamed';

		//выставляем нужный шаблон для RTL режима
		if ($_REQUEST['direction'] == "RTL")
		{
			$_REQUEST['navigationBarSkin'] = "navigation_rtl";
			$_REQUEST['firstPage'] = 9999;
		}
		
		if ( !empty($_FILES['image']['name'][0]) )
			$imageId = $this->upload_image('bgImage');
		else
			$imageId[0] = $_REQUEST['bgImage'];

		
		$_REQUEST['bookName'] = $wpdb->escape( $_REQUEST['bookName'] );
		$imageId[0] = $wpdb->escape( $imageId[0] );

		$sql = "INSERT INTO `{$this->table_name}` (`name`, `date`, `bgImage`) VALUES ('{$_REQUEST['bookName']}', '".date("U")."', '{$imageId[0]}')";
		$wpdb->query($sql);

		$book_id = $wpdb->get_var( "SELECT LAST_INSERT_ID();", 0, 0 );

		
		$_REQUEST['flipSound'] = $this->add_sound();

		
		$newBook = new pageflipBook();

		$newBook->id = $book_id;
		foreach( $newBook->properties as $property )
			if( !empty($_REQUEST[$property]) || $property == 'flipSound' )
				$newBook->$property = $_REQUEST[$property];

		if ($galleryId = $_REQUEST['galleryId'])
		{
	...
	function add_page( $imageId, $type, &$book )
	{
		...
		$imageName = isset($_REQUEST['name']) ? htmlspecialchars(stripslashes( $_REQUEST['name'] )) : NULL;
		$zoomURL = '';
		...
		$book->pages[$book->countPages] = new pageflipPage( $image, $book->countPages, $imageName, $zoomURL );
		$book->countPages++;
	...
	function addGallery($exit = true)
	{
		...
		$name = $wpdb->escape( $_REQUEST['galleryName'] );
	...
	function upload_image( $action='' )
	{
		...
		if( !empty($_REQUEST['url']) && $_REQUEST['uploadFormType']=='uploadFromUrlForm' )
		{
		   	
		   	if ( !$this->functions->isUrl($_REQUEST['url']) )
		   	{
		   		$txt = '<strong>' . $_REQUEST['url'] . '</strong> - <strong>' . __('Error', 'pageFlip') . '</strong>: ' . __('Incorrect url', 'pageFlip') . '<br />';
				echo $this->functions->errorMessage( $txt );
				return false;
		   	}

		   	if ( !$this->functions->checkImage($_REQUEST['url']) )
		   		return false;

		   	$type = 'img';

			if ( empty($_REQUEST['name']) )
				$_REQUEST['name'] = basename( $_REQUEST['url'] );

			$_REQUEST['name'] = $wpdb->escape(htmlspecialchars(stripslashes( $_REQUEST['name'] ))); 
			
			$sql = "INSERT INTO `{$this->table_img_name}` (`name`, `filename`, `date`, `type`, `gallery`) VALUES ('{$_REQUEST['name']}', '{$_REQUEST['url']}', '".date("U")."', '{$type}', '{$_REQUEST['galleryId']}')";
			$res = $wpdb->query($sql);
		...
		if ( $_REQUEST['uploadFormType']=='uploadSwfForm' )
		{
			$_REQUEST['folder'] = str_replace(ABSPATH, '', $this->plugin_path.'upload/');
		}

		if( !empty( $_REQUEST['folder'] ) && ($_REQUEST['uploadFormType']=='uploadFromFolder' || $_REQUEST['uploadFormType']=='uploadSwfForm') )
		{
			ini_set('max_execution_time', 600);

			$curDir = ABSPATH . $_REQUEST['folder'];
			if( is_dir($curDir) )
			{
				$dir = opendir($curDir); 
		...
		if( !empty( $_FILES['zip']['name'] ) && $_REQUEST['uploadFormType']=='uploadZipForm' )
		{
		...
		if ( !empty($_FILES['image']['name']) && $_REQUEST['uploadFormType']=='uploadImgForm' )
		{
			foreach ($_FILES['image']['name'] as $id => $imageName)
			{
				if ( !empty($imageName) )
				{
					...
					$id = $this->copyImage( $imageName, $_FILES['image']['tmp_name'][$id], $_FILES['image']['size'][$id], $action, $_REQUEST['name'][$id], 'move_uploaded_file', $zoomImageName );
					if ($id) $imagesId[] = $id;
				}
			}
		}
	   ...
	   $dir =  $this->plugin_path . $this->imagesDir . '/';
		...
		switch( strtolower( $fileExt['1'] ) )
		{
			case 'swf' :
				...
				break;
			default :
				if( $functionName($tmpName, $new_filename) && $this->functions->img_resize($new_filename, $thumbName, $newSize['width'], $newSize['height']) )
				{
					@chmod($new_filename, 0777);
				}
				else
		...
		$sql = "insert into `".$this->table_img_name."` (`name`, `filename`, `date`, `type`, `gallery`) values ('".$name."', '".basename($new_filename)."', '".date("U")."', '".$type."', '".$_REQUEST['galleryId']."')";
		$wpdb->query( $sql );
	...
	function edit_book()
	{
		...
		if ( empty($_REQUEST['bookName']) ) $_REQUEST['bookName'] = __('Untitled', 'pageFlip');
		...
		else $imageId[0] = $_REQUEST['bgImage'];

		
		$_REQUEST['bookName'] = $wpdb->escape( $_REQUEST['bookName'] );
		$imageId[0] = $wpdb->escape( $imageId[0] );
		$_REQUEST['bookId'] = (int)$_REQUEST['bookId'];

		$sql = "UPDATE `{$this->table_name}` SET `name` = '{$_REQUEST['bookName']}', `bgImage` = '{$imageId[0]}' WHERE `id` = '{$_REQUEST['bookId']}'";
		$wpdb->query($sql);

		
		$_REQUEST['flipSound'] = $this->add_sound();;

		$book = new pageflipBook( $_REQUEST['bookId'] );

		
		foreach( $book->properties as $property )
			if( isset($_REQUEST[$property]) && (string)$_REQUEST[$property] !== '' || $property == 'flipSound' || $property == 'fullscreenHint' )
				$book->$property = $_REQUEST[$property];
	...
	function images_list( $bookId = 0, $gallery = 0 )
	{
		...
		if( (int)$_REQUEST['page'] < 1 ) $_REQUEST['page'] = 1;

		$navigation = $this->functions->navigationBar( $_REQUEST['page'], get_option( 'pageFlip_imgPerPage' ), $_REQUEST['type'], '', $gallery );
		...
		if( $_REQUEST['type'] === 'pageFlip' || $_REQUEST['type'] === 'NGGallery' )
		{
		...
		$list .= $this->html->operationPreviewImage( $bookId, 'top', $navigation, $_REQUEST['type'], $gallery );
		$list .= $this->html->headerPreviewImage();
		$list .= $this->viewImagesList( $bookId, $start, get_option('pageFlip_imgPerPage'), $_REQUEST['type'], $gallery );
		$list .= $this->html->footerPreviewImage();
		$list .= $this->html->operationPreviewImage( $bookId, 'bottom', $navigation, $_REQUEST['type'], $gallery );
	...
	function pagingImages()
	{
		...
		$navigation = $this->functions->navigationBar( $_REQUEST['page'], get_option( 'pageFlip_imgPerPage' ), $_REQUEST['type'] );
		...
		echo $this->viewImagesList( $_REQUEST['bookId'], $start, get_option( 'pageFlip_imgPerPage' ), $_REQUEST['type'], $_REQUEST['gallery'] );
	...
	{
		echo $this->html->addPageMenu( $_REQUEST['type'] );
		echo '<split>';
		echo $this->html->buttonsOpImages( $_REQUEST['bookId'], $_REQUEST['type'] );
	...
	{
		update_option( 'pageFlip_imgPerPage', (int)$_REQUEST['count'] );
		exit;
	}
	...
	function delete_page()
	{
		...
		$book->deletePage( $_REQUEST['pageId'] ); 
	...
	function splitImage()
	{
		...
		if( $this->functions->checkPic( $book->pages[(int)$_REQUEST['pageId']]->image ) != 'pageFlip' ) exit;

		$image = $this->imagesPath . basename( $book->pages[(int)$_REQUEST['pageId']]->image );
		$zoomImage = $this->imagesPath . basename( $book->pages[(int)$_REQUEST['pageId']]->zoomURL );
		...
		$book->pages[(int)$_REQUEST['pageId']] = new pageflipPage( $firstImage, $_REQUEST['pageId'], '', $firstZoomImage );

		for( $i = $book->countPages; $i > $_REQUEST['pageId'] + 1; $i-- )
		{
			$book->pages[$i] = $book->pages[($i - 1)];
			$book->pages[$i]->number = $i;
		}

		$book->pages[($_REQUEST['pageId'] + 1)] = new pageflipPage( $secondImage, ($_REQUEST['pageId'] + 1), '', $secondZoomImage );
	...
	function mergeImage()
	{
		...
		$secondImage = substr( basename( $book->pages[(int)$_REQUEST['pageId']]->image ), 2 );
		$mergeImage = $this->imagesUrl . $this->functions->getSplitImageName( $book->pages[(int)$_REQUEST['pageId']]->image );

		@unlink( $this->imagesPath . basename( $book->pages[(int)$_REQUEST['pageId']]->image ) ); 
		@unlink( $this->imagesPath . 't_' . basename( $book->pages[(int)$_REQUEST['pageId']]->image ) ); 

		
		$book->pages[(int)$_REQUEST['pageId']] = new pageflipPage( $mergeImage, $_REQUEST['pageId'], '' );
	...
	{
		if( empty( $_REQUEST['imageList'] ) ) return false;

		$images = split( ';', $_REQUEST['imageList'] );
	...
	function deleteGallery()
	{
		...
		$sql = "select `id` from `".$this->table_img_name."` where `type` = 'img' and `gallery` = '".$_REQUEST['gallery']."'";
		$images = $wpdb->get_results($sql, ARRAY_A);
		if( count( $images ) > 0 )
			foreach( $images as $img ) $this->delete_image( $img['id'] );

		
		$sql = "delete from `".$this->table_gal_name."` where `id` = '".$_REQUEST['gallery']."'";
	...
	function moveImgTo( $galleryId = '', $imageId = '' )
	{
		...
		{
			$galleryId = (int)$_REQUEST['gallery'];
	...
	function moveImgsTo()
	{
		...
		foreach( $images as $imageId )
			$this->moveImgTo( $_REQUEST['gallery'], $imageId );
	...
	function add_sound()
	{
		if ($_FILES['sound']['name'])
		{
		   ...
		   $_REQUEST['flipSound'] = basename($new_filename);

		   if(!copy( $_FILES['sound']['tmp_name'], $new_filename ) ) {echo __("Write file error!", 'pageFlip'); return '';}
		}

		if( $_REQUEST['flipSound'] !== '' ) $flipSound = $this->plugin_url . $this->soundsDir . '/' . $_REQUEST['flipSound']; 
		else $flipSound = '';
	...
	{
		echo $this->add_page_form( $_REQUEST['bookId'], $_REQUEST['imageId'], $_REQUEST['type'] );
		exit;
	}
	...
	function flashEditor( $do, $exit = true )
	{
		...
		{
			case 'loadalbumxml':
				echo $this->functions->loadAlbumXml( (int)$_POST['bookId'] );
				break;
			case 'savealbumxml':
				$this->functions->saveAlbumXml( (int)$_POST['bookId'] );
				break;
			case 'loadlayouts':
				echo $this->functions->loadLayouts( );
				break;
		}
		if ( $exit )
			exit();
	...
	function book_form( $bookId = '', $galleryId = '' )
	{
		...
		$thisBook = new pageflipBook( $bookId );
...
';
	...
	function pageFlipWidgetControl($widget_args) {
		...
		if ( !$updated && !empty($_REQUEST['sidebar']) ) {
			$sidebar = (string) $_REQUEST['sidebar'];
			...
			foreach ( (array) $this_sidebar as $_widget_id ) {
				if ( array($this, 'pageFlipWidget') === $wp_registered_widgets[$_widget_id]['callback'] && isset($wp_registered_widgets[$_widget_id]['params'][0]['number']) ) {
					...
					if ( !in_array( "pageflip-$widget_number", $_REQUEST['widget-id'] ) ) 
						unset($options[$widget_number]);
				}
			}

			foreach ( (array)$_REQUEST['widget-pageflip'] as $widget_number => $widget_pageflip ) {
