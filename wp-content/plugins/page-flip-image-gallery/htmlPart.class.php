...
{$pagesLink}
			...
			flippingBook{$id}.settings.direction = \"{$book->direction}\";
...
";
	...
	<div style=\"width:{$width}; text-align:center; font-size:9px;\">
		<a href=\"http://readyshoppingcart.com/wordpress-gallery-plugin/\" title=\"WordPress Gallery Plugin\">WordPress Gallery Plugin</a>
	</div>
...
";
				...
				//<![CDATA[
					...
					function viewEditor(bookId) {
						(function($) {
							...
							swfobject.embedSWF('{$this->main->editor}', 'bookList', '100%', height, '9', '', {
								album_id: bookId,
								lang: 'en_US',
								root_path: '{$rootUrl}'
							});
   ...
   function operationBookPreview( $side = 'top' )
   {
	...
	function previewBook( $book, $bookName, $creationDate, $firstPageImg, $secondPageImg )
	{
		...
		$text .= "							<input type=\"hidden\" name=\"action\" value=\"Delete Book\" />
							...
							<input type=\"submit\" class=\"button\" name=\"do\" value=\"{$tAccess}\" />
					    </form>
					  </td>
				   </tr>
				   <tr>
   ...
   function addPageForm( $id, $imageId, $image, $imageName, $type = 'pageFlip' )
   {
	...
	function uploadImgForm()
	{
        ...
        $tImage = __('Image', 'pageFlip');
        $tZoomImage = __('Zoom Image', 'pageFlip');
        $tImageDesc = __('Image must be jpg, png, gif file', 'pageFlip');
        $tZoomImageDesc = __('Large image', 'pageFlip');
...
';
	...
	function bookForm( $title, $name, $book, $bgImage, $bgImagesAr, $flipSound, $bgImageList, $action, $button, $galleryId = '' )
	{
		...
		$d_0 = ($book->direction == 'LTR') ? ' checked="checked"' : '';
		$d_1 = ($book->direction == 'RTL') ? ' checked="checked"' : '';
		...
		$tLTR = __('Left to right', 'pageFlip');
		$tRTL = __('Right to left', 'pageFlip');
			...
			<form name=\"addbook\" id=\"addbook\" method=\"post\" action=\"\" enctype=\"multipart/form-data\" class=\"add:the-list: validate\">
				...
				<table class=\"form-table\">
					<tbody>
						...
						<tr class=\"form-field\" style=\"border-bottom: 1px solid #ccc; background-color: #eee;\">
							<th scope=\"row\" valign=\"top\"><label for=\"direction\">{$tDirection}</label></th>
							<td id=\"direction\">
							  <label><input type=\"radio\" name=\"direction\" value=\"LTR\" id=\"direction_0\"{$d_0} style=\"width:10px; height:10px;\" /> {$tLTR}</label>&nbsp;
							  <label><input type=\"radio\" name=\"direction\" value=\"RTL\" id=\"direction_1\"{$d_1} style=\"width:10px; height:10px;\" /> {$tRTL}</label>
							  <p>{$tDirectionDesc}</p>
							</td>
						</tr>
						<tr class=\"form-field\" style=\"border-bottom: 1px solid #ccc;\">
