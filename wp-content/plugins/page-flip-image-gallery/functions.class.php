...
class pageflipFunctions
{
   ...
   function pageflipFunctions()
   {
	...
	{
	  	$imageName = basename( $imageName );
	  	if (!preg_match('/.*\.(jpg)|(jpeg)|(gif)|(png)$/i', $imageName, $m))
	  	{
                        echo '<div class="error" id="error"><p><strong>'.$imageName.'</strong> - <strong>' . __('Error', 'pageFlip') . '</strong>:' . __('Wrong file type', 'pageFlip') . '<br></p></div>';
			return false;
	  	}
