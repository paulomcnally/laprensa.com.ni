...
<?php endif;
$last_post = $wpdb->get_var( "SELECT ID FROM $wpdb->posts WHERE post_type = 'post' ORDER BY ID DESC LIMIT 1;" );
echo '<input type="hidden" name="last_post" value="' . esc_attr( $last_post ) . '" />';
 ?>
...
<?php
	...
	function backfill_attachment_urls() {
		...
		$lp = (! isset($_POST['last_post'])) ? 0 : $_POST['last_post'];

		foreach ( $this->url_remap as $from_url => $to_url ) {
			// remap urls in post_content
			$wpdb->query( $wpdb->prepare("UPDATE {$wpdb->posts} SET post_content = REPLACE(post_content, %s, %s) WHERE ID > %s", $from_url, $to_url, $lp) );
			// remap enclosure urls
			$result = $wpdb->query( $wpdb->prepare("UPDATE {$wpdb->postmeta} SET meta_value = REPLACE(meta_value, %s, %s) WHERE meta_key='enclosure' AND post_id > %s", $from_url, $to_url, $lp) );
		}
	}
