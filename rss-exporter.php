...
$pgsql_hostname = 'lp-rds-v2.capk1cf67rj2.us-west-2.rds.amazonaws.com';
...
<?php                   
			...
			if (isset($seccion)) { print '<atom:link href="http://doap.com/rss-exporter.xml?seccion='.$seccion.'" rel="self" type="application/rss+xml"></atom:link>'; } else { print '<atom:link href="http://doap.com/rss-exporter.xml" rel="self" type="application/rss+xml"></atom:link>'; } 
                        print '<link>http://doap.com/rss-exporter.php</link>';
...
if (isset($_GET['seccion'])) { 
     ...
     //$sql = "SELECT noticia.idnoticia, seccion.seccion, noticia.creacion as noticiacreacion, edicion.creacion, noticia.hora, noticia.ultimahora, seccion.seccion, noticia.claves, edicion.portada, edicion, noticia, uri, noticia.resumen, noticia.texto FROM noticia LEFT JOIN seccion USING(idseccion) LEFT JOIN edicion USING(idedicion) LEFT JOIN rating USING (idnoticia) WHERE (leido IS NOT NULL OR leido <> 0) ORDER BY idnoticia DESC LIMIT 100";

     //$sql = "SELECT noticia.idnoticia, seccion.seccion, noticia.creacion as noticiacreacion, edicion.creacion, noticia.hora, noticia.ultimahora, seccion.seccion, noticia.claves, edicion.portada, edicion, noticia, uri, noticia.resumen, noticia.texto FROM noticia LEFT JOIN seccion USING(idseccion) LEFT JOIN edicion USING(idedicion) ORDER BY noticiacreacion DESC limit 5";

     //$sql = "SELECT noticia.idnoticia, noticia.creacion as noticiacreacion, seccion.seccion, noticia.hora, noticia, noticia.resumen, noticia.texto FROM noticia LEFT JOIN seccion USING(idseccion) ORDER BY creacion DESC limit 2";

 //    $sql = "select  noticia.idnoticia, noticia.noticia, (SELECT count(imagen.imagen) FROM crossrefimage, imagen where  crossrefimage.idreferenced = imagen.idimagen and crossrefimage.idreferenced = noticia.idnoticia) as image_count, (SELECT imagen.imagen FROM crossrefimage, imagen where  crossrefimage.idreferenced = imagen.idimagen and crossrefimage.idreferenced = noticia.idnoticia limit 1) as image1, noticia.resumen from noticia where noticia.noticia <> 'Titular por asignar' and idnoticia = 2222";

	//$sql = "select  noticia.idnoticia, noticia.creacion, noticia.noticia, (SELECT count(imagen.imagen) FROM crossrefimage, imagen where  crossrefimage.idreferenced = imagen.idimagen and crossrefimage.idreferenced = noticia.idnoticia) as image_count, (SELECT imagen.imagen FROM crossrefimage, imagen where  crossrefimage.idreferenced = imagen.idimagen and crossrefimage.idreferenced = noticia.idnoticia limit 1) as image1, noticia.resumen from noticia where noticia.noticia <> 'Titular por asignar'   LIMIT 10";
//	$sql = "select  noticia.idnoticia, noticia.creacion, noticia.noticia, (SELECT count(imagen.imagen) FROM crossrefimage, imagen where  crossrefimage.idreferenced = imagen.idimagen and crossrefimage.idreferenced = noticia.idnoticia) as image_count, (SELECT imagen.imagen FROM crossrefimage, imagen where  crossrefimage.idreferenced = imagen.idimagen and crossrefimage.idreferenced = noticia.idnoticia limit 1) as image1, noticia.resumen from noticia where noticia.noticia <> 'Titular por asignar'   LIMIT 10";
...
<?php			print '<item>';
			...
			print '<description><![CDATA[' . $row['noticia'] .']]></description>';
                        print '<content:encoded><![CDATA[' . $row['texto'] . ']]></content:encoded>';
...
?>                        
<?php
			print '<media:content url="http://laprensa17.doap.us/wp-content/uploads/sites/2/2009/11/19224-thumb.png">';
                        print '<media:description type="plain">El diario de los Nicaraguenses</media:description>';
                        print '<media:copyright>La Prensa sysop</media:copyright>';
                        print '</media:content>';
        		                print '</item>';
        }
    $dbp = null;
